...
-totalled changed K10 pro/ISO/RGB/VIA/keymap.c, removed mac, layer 5 exclusively for FN
-FN+esc = flashing mode even when plugged with UBS when on bluetooth
...
Changed macro KC_TRNS to fetch the key of layer 0 of that button instead, so the same macro can be used on many different buttons.
Switched F21-24 to some text check instead along with other added features.

- \[init] = same as having F21, start of the loop
- \[loop] = same as F22, go back to [init] if key is held
- \[toggle] = same as F23, turn the macro off
- \[stopall] = same as F24, turn off all macro, except the one that did \[stopall]
- \[stop] = stop the macro if the key is not held.
- \[end] = stop the current macro running [end]
...
- \[;0] = will jump the amount of steps after the ; , this can also be applied to ctrl,alt or shift as such \
	\[ctrl;10] so if ctrl is helf, it will jump 10 steps in the data else it will continue the macro normally, usefull to make macro work in different ways based on button held,
	it's very annoying to count the amount of steps needed based on what you're doing but a keypress or release is 3 steps, delay is 2-5 based on the amount of numbers, text is 1 per character
